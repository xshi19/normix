...
[`../dev-notes/archive/plans/asv_benchmarking.md`](../dev-notes/archive/plans/asv_benchmarking.md).
...
`--python=same` cannot compare two commits (`install_project` is a no-op).
PR tripwire (isolated uv env, CPU-only Bessel / GIGFromExpectation):

```bash
cd asv_bench
uv run asv continuous --factor 1.5 --split \
  -b 'Bessel|GIGFromExpectation' master HEAD
```

Two device series share one install (`install_command` + `_asv_install.py`
adds `jax[cuda12]` on Linux **with a GPU**). `JAX_PLATFORMS` is `env_nobuild`:
cpu and cuda are separate trend lines, no rebuild. Non-linux has no
`jax[cuda12]` wheels; `exclude` drops the cuda series there. CPU-only Linux
(GHA) skips the cuda extra and the cuda series (`NotImplementedError` in
setup).
...
| `_asv_install.py` | `install_command`: wheel `[cuda12]` + Linux `jax[cuda12]==0.9.1` when a GPU is present |
| `benchmarks/__init__.py` | skip cuda series when plugin or NVIDIA device is missing |
