...
- **E-step**: `model.e_step(X, backend='jax'|'cpu') -> EStepResult`
  ...
  - Returns `EStepResult(eta, log_lik)`: `eta` is a `NormalMixtureEta` pytree (6 aggregated expectation fields); `log_lik` is the conjugacy mean $\ell_n(\theta_t)$ from `BesselMoments.log_k` (no extra Bessel call). Not a field of `NormalMixtureEta`, not a cache on the model.
...
- **Batch fitter**: `BatchEMFitter` is a plain Python class (not an `eqx.Module`). Its `fit(model, X)` returns **`EMResult`**: fitted `model`, optional per-iteration conjugacy log-likelihoods, `param_changes` (hybrid-RMS diagnostic on $\mu$, $\gamma$, $L_\Sigma$), `n_iter`, `converged` (Aitken remaining gap of $\ell_n$ in nats/obs; E3), `elapsed_time`. Optional `eta_update` rule enables penalised / shrinkage batch EM. Incremental EM has no Aitken stop (`converged=None`).
...
| `asv_bench/` | ASV trend-tracking micro-suite (Bessel, GIG, E-step, EM, compile, sampling; cpu+cuda via `JAX_PLATFORMS`). Dashboard published at `/benchmarks/`. PR tripwire: `.github/workflows/asv.yml`. Deep-dive scripts stay in `benchmarks/`. Plan: `archive/plans/asv_benchmarking.md` |
