...
from normix_numpy.base import JointNormalMixture, ExponentialFamily
from normix_numpy.distributions.univariate import InverseGamma
from normix_numpy.params import NormalInverseGammaParams
from normix_numpy.utils import robust_cholesky
...
class JointNormalInverseGamma(JointNormalMixture):
    ...
    _alpha : float or None
        InverseGamma shape parameter :math:`\\alpha`.
    _beta : float or None
        InverseGamma rate parameter :math:`\\beta`.
    ...
    def _compute_expectation_params(self) -> NDArray:
        ...
        Avoids the round-trip through natural parameters.
        """
        mu = self._mu
        gamma = self._gamma
        Sigma = self._L_Sigma @ self._L_Sigma.T
        ...
        E_inv_Y = alpha / beta
        E_Y = beta / (alpha - 1) if alpha > 1 else np.inf
