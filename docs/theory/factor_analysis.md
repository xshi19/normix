...
where $L_{GIG}$ is the GIG log-likelihood defined in {eq}`gig-loglik`.

## M-Step: Closed-Form Solutions

Setting the partial derivatives of $L_{FA}$ to zero yields analytic
...
Integrating {eq}`fa-joint` over $z$, the joint distribution of
...
$E[\log Y | X, u]$ are computed using {eq}`gig-moment-cond`.
