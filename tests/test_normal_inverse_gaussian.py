"""
Tests for Normal Inverse Gaussian distributions (current API).
...
class TestJointNormalInverseGaussian:

    def test_construction(self):
        j = _joint_1d()
        assert j.d == 1
        np.testing.assert_allclose(float(j.mu_ig), 1.0)
        np.testing.assert_allclose(float(j.lam), 1.0)
    ...
    def test_rvs_y_mean(self):
        """Y ~ InvGaussian(μ_ig, λ), so E[Y] = μ_ig."""
        mu_ig, lam = 2.0, 3.0
        ...
        np.testing.assert_allclose(float(Y.mean()), mu_ig, rtol=0.05)
...
class TestNormalInverseGaussian:
...
class TestNormalInverseGaussianEdgeCases:
    ...
    def test_large_lam(self):
        """Large λ concentrates IG around μ_ig."""
        nig = NormalInverseGaussian.from_classical(
            mu=jnp.array([0.0]), gamma=jnp.array([0.5]),
            sigma=jnp.array([[1.0]]), mu_ig=1.0, lam=10.0,
        )
        X = nig.rvs(20, seed=42)
        lps = jax.vmap(nig.log_prob)(X)
        assert jnp.all(jnp.isfinite(lps))

    def test_small_lam(self):
        """Small λ gives heavy tails."""
        nig = NormalInverseGaussian.from_classical(
            mu=jnp.array([0.0]), gamma=jnp.array([0.5]),
            sigma=jnp.array([[1.0]]), mu_ig=1.0, lam=0.5,
        )
        X = nig.rvs(20, seed=42)
        lps = jax.vmap(nig.log_prob)(X)
        assert jnp.all(jnp.isfinite(lps))
